...
bool GetComputerList(Settings& settings, LPCWSTR& cmdLine)
{
	...
	else
	{
		if(L'@' == *cmdLine)
		{
			...
			while(!iswspace(*cmdLine) && *cmdLine)
				cmdLine++;
			CString file = CString(fileStart).Left(int(cmdLine - fileStart));
		...
		else
		{
			if(0 == wcsncmp(cmdLine, L"\\\\", 2))
			{
				...
				while(!iswspace(*cmdLine) && *cmdLine)
					cmdLine++;
				CString compList = CString(compListStart).Left(int(cmdLine - compListStart));
...
#define PW_BUFF_LEN 500
		...
		if(L'"' == *pAppStart)
		{
			...
			if(NULL == pArgPtr)
				return false;
			settings.app = CString(pAppStart).Left(int(pArgPtr - pAppStart));
